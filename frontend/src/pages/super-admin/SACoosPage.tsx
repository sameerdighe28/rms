...
export default function SACoosPage() {
  ...
  const [form, setForm] = useState({ email: '', password: '', fullName: '', mobileNumber: '', role: 'COO' });
  const [loading, setLoading] = useState(false);

  const load = async () => {
    try { setCoos((await superAdminApi.getCoos()).data); }
    catch { toast.error('Failed to load COOs'); }
  };
  ...
  const handleCreate = async (e: FormEvent) => {
    ...
    try {
      ...
      setShowModal(false);
      setForm({ email: '', password: '', fullName: '', mobileNumber: '', role: 'COO' });
      load();
    ...
    <>
      ...
      {coos.length === 0 ? (
        ...
        <div className="table-wrap"><table>
          <thead><tr><th>Name</th><th>Email</th><th>Role</th><th>Company</th></tr></thead>
          <tbody>
            {coos.map(u => (
              <tr key={u.id}>
                <td><strong>{u.fullName}</strong></td>
                <td>{u.email}</td>
                <td><span className="badge badge-technical">{u.role}</span></td>
                <td>{u.companyName || '—'}</td>
              </tr>
            ))}
          </tbody>
